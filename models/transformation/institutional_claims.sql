...
,   row_number() over(partition by a.cur_clm_uniq_id, a.data_source order by b.clm_line_num) as claim_row_number
, a.data_source
from {{ ref('parta_claims_header')}} a
left join {{ ref('parta_claims_revenue_center_detail')}} b
...
, a.data_source
from claim_line a
inner join {{ ref('parta_claims_header')}} b
...
select
    ...
    , cast( a.data_source as {{ dbt.type_string() }} ) as data_source
from claim_line_a a
left join {{ ref('parta_claims_header')}} h
  on a.claim_id = h.cur_clm_uniq_id
  and a.data_source = h.data_source
left join {{ ref('parta_claims_revenue_center_detail')}} d
...
left join {{ ref('procedure_pivot')}} px
	on a.claim_id = px.cur_clm_uniq_id
            and a.data_source = px.data_source
left join {{ ref('diagnosis_pivot')}} dx
	on a.claim_id = dx.cur_clm_uniq_id
            and a.data_source = dx.data_source